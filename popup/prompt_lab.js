function renderPromptLabUI() {
        ...
        <div class="prompt-lab-container no-scrollbar" style="display: flex; flex-direction: column; min-height: 100%; padding: 20px; box-sizing: border-box; overflow-y: auto; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
            <div style="margin-bottom: 20px; flex-shrink: 0;">
                <h2 style="margin: 0 0 6px 0; font-size: 22px; font-weight: 700; display: flex; align-items: center; gap: 8px; color: var(--text-color); letter-spacing: -0.5px;">
                    Prompt Lab
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="color: #0A84FF;"><path d="M6 18h8"/><path d="M3 22h18"/><path d="M14 22a7 7 0 1 0 0-14h-1"/><path d="M9 14h2"/><path d="M9 12a2 2 0 0 1-2-2V6h6v4a2 2 0 0 1-2 2Z"/><path d="M12 6V3a1 1 0 0 0-1-1H9a1 1 0 0 0-1 1v3"/></svg>
                </h2>
    ...
    const pullCurrentPromptText = async () => {
        try {
            ...
            if (result && result.trim().length > 0) {
                draftInput.value = result.trim();
                return result.trim();
            }
        } catch(err) {
            console.log("Prompt Lab Pull Error (Host might not be loaded):", err);
        }
        return "";
    };
    ...
    const pushToPage = async (text) => {
        try {
            ...
            console.log("Prompt Lab Push Error:", err);
            navigator.clipboard.writeText(text);
            return false;
        }
    };
    ...
    enhanceBtn.addEventListener("click", async () => {
        ...
        if (!draft) {
            draft = await pullCurrentPromptText();
        }
        ...
        const finishEnhance = (finalContext) => {
            ...
            chrome.runtime.sendMessage({
                ...
                if (response && response.success) {
                    ...
                    // 2. Auto-push to page
                    pushToPage(finalOutput).then(pushed => {
