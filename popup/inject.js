...
function handleUseWithText(text) {
    if (!text) return;
...
// Listen for injection requests from the Side Panel
chrome.runtime.onMessage.addListener((req, sender, sendResponse) => {
    if (req.type === "INJECT_PROMPT") {
        handleUseWithText(req.text);
        sendResponse({ success: true });
    } else if (req.type === "GET_HOST_TEXT") {
        let result = "";
        try {
            ...
            console.error("GET_HOST_TEXT Error", e);
        }
        sendResponse({ success: true, text: result || "" });
    } else if (req.type === "SET_HOST_TEXT") {
        const text = req.text;
